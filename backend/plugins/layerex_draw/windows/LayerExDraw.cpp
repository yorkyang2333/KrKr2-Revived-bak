...
Image *loadImage(const tjs_char *name) {
    ...
    if(filename.length()) {
        if(false &&
           !wcschr(filename.toWString().c_str(),
                   '>')) { // にぎりっぱになるのでやめ
            // 実ファイルが存在
            TVPGetLocalName(filename);
            image = Image::FromFile(filename.toWString().c_str(), false);
        } else {
                ...
                if(hBuffer) {
                    ...
                    if(pBuffer) {
                        if(in->Read(pBuffer, size, &size) == S_OK) {
                            ...
                            if(::CreateStreamOnHGlobal(hBuffer, FALSE,
                                                       &pStream) == S_OK) {
...
void GdiPlus::addPrivateFont(const tjs_char *fontFileName) {
    ...
    if(filename.length()) {
        if(!wcschr(filename.toWString().c_str(), '>')) {
            // 実ファイルが存在
            TVPGetLocalName(filename);
            privateFontCollection->AddFontFile(filename.toWString().c_str());
            return;
        } else {
            // メモリにロードして展開
            IStream *in = TVPCreateIStream(filename, TJS_BS_READ);
 ...
 */
FontInfo::FontInfo() :
    fontFamily(NULL), emSize(12), style(0), gdiPlusUnsupportedFont(false),
...
FontInfo::FontInfo(const tjs_char *familyName, REAL emSize, INT style) :
    fontFamily(NULL), gdiPlusUnsupportedFont(false), forceSelfPathDraw(false),
...
bool FontInfo::getForceSelfPathDraw(void) const { return forceSelfPathDraw; }

bool FontInfo::getSelfPathDraw(void) const {
    return forceSelfPathDraw || gdiPlusUnsupportedFont;
}

OUTLINETEXTMETRIC *FontInfo::createFontMetric(void) const {
    ...
    if(size > 0) {
        ...
        if(::GetOutlineTextMetrics(
               dc, size, reinterpret_cast<OUTLINETEXTMETRIC *>(buf))) {
        ...
        switch(type) {
            case BrushTypeSolidColor:
                brush = new SolidBrush(
                    Color((ARGB)info.getIntValue(TJS_W("color"), 0xffffffff)));
                break;
            case BrushTypeHatchFill:
                brush = new HatchBrush(
                    ...
                    Color(
                        (ARGB)info.getIntValue(TJS_W("foreColor"), 0xffffffff)),
                    Color((ARGB)info.getIntValue(TJS_W("backColor"),
                                                 0xff000000)));
                break;
            case BrushTypeTextureFill: {
                ...
                if(image) {
                    ...
                    if(info.checkVariant(TJS_W("dstRect"), dstRect)) {
                        brush =
                            new TextureBrush(image, wrapMode, getRect(dstRect));
                    } else {
            ...
            case BrushTypeLinearGradient: {
                ...
                if(info.checkVariant(TJS_W("point1"), var)) {
                    ...
                    lbrush =
                        new LinearGradientBrush(point1, point2, color1, color2);
                    ...
                    if(info.HasValue(TJS_W("angle"))) {
                        ...
                        lbrush = new LinearGradientBrush(
                            ...
                            (BOOL)info.getIntValue(TJS_W("isAngleScalable"),
                                                   0));
                    } else {
                        // 無い場合はモードを参照
                        lbrush = new LinearGradientBrush(
...
bool Appearance::getLineCap(tTJSVariant &in, LineCap &cap,
...
LayerExDraw::LayerExDraw(DispatchT obj) :
    ...
    bitmap(NULL), graphics(NULL), _pClipLeft(obj, TJS_W("clipLeft")),
    _pClipTop(obj, TJS_W("clipTop")), _pClipWidth(obj, TJS_W("clipWidth")),
    _pClipHeight(obj, TJS_W("clipHeight")), clipLeft(-1), clipTop(-1),
    clipWidth(-1), clipHeight(-1), smoothingMode(SmoothingModeAntiAlias),
    textRenderingHint(TextRenderingHintAntiAlias), metaHDC(NULL),
    metaBuffer(NULL), metaStream(NULL), metafile(NULL), metaGraphics(NULL),
    updateWhenDraw(true) {
...
void LayerExDraw::reset() {
    ...
    if(!(graphics && width == _width && height == _height && pitch == _pitch &&
        ...
        pitch = _pitch;
        buffer = _buffer;
        bitmap = new Bitmap(width, height, pitch, PixelFormat32bppARGB,
 ...
 * @param path 描画するパス
 */
RectF LayerExDraw::getPathExtents(const Appearance *app,
...
void LayerExDraw::draw(Graphics *graphics, const Pen *pen, const Matrix *matrix,
                       const GraphicsPath *path) {
...
void LayerExDraw::fill(Graphics *graphics, const Brush *brush,
                       const Matrix *matrix, const GraphicsPath *path) {
    GraphicsContainer container = graphics->BeginContainer();
    graphics->MultiplyTransform(matrix);
    graphics->SetSmoothingMode(smoothingMode);
 ...
 * @return 更新領域情報
 */
RectF LayerExDraw::drawPath(const Appearance *app, const GraphicsPath *path) {
    // 領域記録用
    RectF rect;
 ...
 * @return 更新領域情報
 */
RectF LayerExDraw::drawArc(const Appearance *app, REAL x, REAL y, REAL width,
 ...
 * @return 更新領域情報
 */
RectF LayerExDraw::drawBezier(const Appearance *app, REAL x1, REAL y1, REAL x2,
 ...
 * @return 更新領域情報
 */
RectF LayerExDraw::drawPie(const Appearance *app, REAL x, REAL y, REAL width,
                           REAL height, REAL startAngle, REAL sweepAngle) {
    GraphicsPath path;
 ...
 * @return 更新領域情報
 */
RectF LayerExDraw::drawEllipse(const Appearance *app, REAL x, REAL y,
 ...
 * @return 更新領域情報
 */
RectF LayerExDraw::drawLine(const Appearance *app, REAL x1, REAL y1, REAL x2,
                            REAL y2) {
    GraphicsPath path;
 ...
 * @return 更新領域情報
 */
RectF LayerExDraw::drawRectangle(const Appearance *app, REAL x, REAL y,
                                 REAL width, REAL height) {
    GraphicsPath path;
...
RectF LayerExDraw::drawPathString(const FontInfo *font, const Appearance *app,
    ...
    // 文字列のパスを準備
    GraphicsPath path;
    path.AddString(ttstr{ text }.toWString().c_str(), -1, font->fontFamily,
                   font->style, font->emSize, PointF(x, y),
...
RectF LayerExDraw::drawString(const FontInfo *font, const Appearance *app,
    ...
    bool first = true;
    vector<Appearance::DrawInfo>::const_iterator i = app->drawInfos.begin();
    const wchar_t *wText = ttstr{ text }.toWString().c_str();
    while(i != app->drawInfos.end()) {
        if(i->info) {
            if(i->type == 1) { // ブラシのみ
                Brush *brush = (Brush *)i->info;
                PointF p(x + i->ox, y + i->oy);
                graphics->DrawString(wText, -1, &f, p,
                                     StringFormat::GenericDefault(), brush);
                if(metaGraphics) {
                    metaGraphics->DrawString(wText, -1, &f, p,
                                             StringFormat::GenericDefault(),
                                             brush);
                }
                // 更新領域計算
                if(first) {
                    graphics->MeasureString(wText, -1, &f, p,
                                            StringFormat::GenericDefault(),
                                            &rect);
...
RectF LayerExDraw::measureString(const FontInfo *font, const tjs_char *text) {
    ...
    graphics->SetTextRenderingHint(textRenderingHint);
    Font f(font->fontFamily, font->emSize, font->style, UnitPixel);
    graphics->MeasureString(ttstr{ text }.toWString().c_str(), -1, &f,
...
RectF LayerExDraw::measureStringInternal(const FontInfo *font,
    ...
    graphics->SetTextRenderingHint(textRenderingHint);
    Font f(font->fontFamily, font->emSize, font->style, UnitPixel);
    graphics->MeasureString(wText, -1, &f, PointF(0, 0),
                            StringFormat::GenericDefault(), &rect);
    CharacterRange charRange(0, INT(wcslen(wText)));
    StringFormat stringFormat = StringFormat::GenericDefault();
    stringFormat.SetMeasurableCharacterRanges(1, &charRange);
    Region region;
    graphics->MeasureCharacterRanges(wText, -1, &f, rect, &stringFormat, 1,
                                     &region);
    RectF regionBounds;
 ...
 * @return 更新領域情報
 */
RectF LayerExDraw::drawImageRect(REAL dleft, REAL dtop, Image *src, REAL sleft,
 ...
 * @return 更新領域情報
 */
RectF LayerExDraw::drawImageStretch(REAL dleft, REAL dtop, REAL dwidth,
                                    REAL dheight, Image *src, REAL sleft,
                                    REAL stop, REAL swidth, REAL sheight) {
 ...
 * @return 更新領域情報
 */
RectF LayerExDraw::drawImageAffine(Image *src, REAL sleft, REAL stop,
...
#define AFFINEY(x, y) B *x + D *y + F
        ...
        graphics->DrawImage(src, points, 3, sleft, stop, swidth, sheight,
                            UnitPixel, NULL, NULL, NULL);
        if(metaGraphics) {
            metaGraphics->DrawImage(src, points, 3, sleft, stop, swidth,
                                    sheight, UnitPixel, NULL, NULL, NULL);
        }
...
bool LayerExDraw::redraw(Image *image) {
    if(image) {
        RectF *bounds = getBounds(image);
        if(metaGraphics) {
            metaGraphics->Clear(Color(0));
            metaGraphics->ResetTransform();
            metaGraphics->DrawImage(image, bounds->X, bounds->Y, bounds->Width,
                                    bounds->Height);
            metaGraphics->SetTransform(&transform);
        }
        graphics->Clear(Color(0));
        graphics->SetTransform(&viewTransform);
        graphics->DrawImage(image, bounds->X, bounds->Y, bounds->Width,
                            bounds->Height);
        graphics->SetTransform(&calcTransform);
        delete bounds;
        _pUpdate(0, NULL);
...
bool LayerExDraw::saveRecord(const tjs_char *filename) {
    ...
    if(metafile) {
        ...
        ULONG size;
        // ファイルに書き出す
        if(metaBuffer && (size = GlobalSize(metaBuffer)) > 0) {
...
void LayerExDraw::getGlyphOutline(const FontInfo *fontInfo, PointF &offset,
                                  GraphicsPath *path, UINT glyph) {
    static const MAT2 mat = { { 0, 1 }, { 0, 0 }, { 0, 0 }, { 0, 1 } };

    GLYPHMETRICS gm;

    int size = GetGlyphOutlineW(metaHDC, glyph,
                                GGO_BEZIER, //  | GGO_GLYPH_INDEX,
                                &gm, 0, NULL, &mat);
    char *buffer = NULL;
    if(size > 0) {
        buffer = new char[size];
        int result = GetGlyphOutlineW(metaHDC, glyph,
                                      GGO_BEZIER, //  | GGO_GLYPH_INDEX,
                                      &gm, size, buffer, &mat);
        if(result <= 0) {
            delete[] buffer;
        ...
        GetGlyphOutlineW(metaHDC, glyph, GGO_METRICS, &gm, 0, NULL, &mat);
    }
...
void LayerExDraw::getTextOutline(const FontInfo *fontInfo, PointF &offset,
    ...
    wcscpy_s(font.lfFaceName,
             ttstr{ fontInfo->familyName }.toWString().c_str());
