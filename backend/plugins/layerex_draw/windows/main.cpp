...
NCB_REGISTER_SUBCLASS_DELAY(RectF) {
    ...
    NCB_METHOD_DETAIL(InflatePoint, Class, void, Class::Inflate,
                      (const PointF &));
    // XXX	NCB_METHOD_DETAIL(Intersect, Class, BOOL, Class::Intersect, (const
    // Rect&));
    NCB_METHOD(IntersectsWith);
    NCB_METHOD(IsEmptyArea);
    NCB_METHOD_DETAIL(Offset, Class, void, Class::Offset, (REAL, REAL));
    // XXX	NCB_METHOD_DETAIL(OffsetPoint, Class, void, Class::Offset, (const
    // Point&));
    NCB_METHOD(Union);
};
...
static tjs_error MatrixFactory(GdipWrapper<Matrix> **result, tjs_int numparams,
    ...
    if(numparams == 0) {
        ...
        matrix =
            new Matrix((REAL)params[0]->AsReal(), (REAL)params[1]->AsReal(),
 ...
 */
template <typename>
struct ImageConvertor : public GdipTypeConvertor<Image> {
    void operator()(GdipClassP &dst, const tTJSVariant &src) {
        if(src.Type() == tvtObject) {
            WrapperT *obj;
            if((obj = AdaptorT::GetNativeInstance(src.AsObjectNoAddRef()))) {
...
static tjs_error ImageFactory(GdipWrapper<Image> **result, tjs_int numparams,
                              tTJSVariant **params, iTJSDispatch2 *objthis) {
    if(numparams == 0) {
        *result = new GdipWrapper<Image>();
        return TJS_S_OK;
    }
    if(numparams > 0 && params[0]->Type() == tvtString) {
