...
public:
	tjs_error FuncCall(
		...
		NI_LayerExBase *dest;
		if ((dest = NI_LayerExBase::getNative(objthis)) == nullptr) return TJS_E_NATIVECLASSCRASH;
		dest->reset(objthis);
		
		NI_LayerExBase *src;
		if ((src = NI_LayerExBase::getNative(param[0]->AsObjectNoAddRef())) == nullptr) return TJS_E_NATIVECLASSCRASH;
...
void InitPlugin_Perspective() {
    ...
    //
    // 			    // 専用メソッドの追加
    // 			    addMethod(dispatch, TJS_W("perspectiveCopy"), new
    // tPerspectiveCopy());
    // 		    }
    //
    // 		    global->Release();
