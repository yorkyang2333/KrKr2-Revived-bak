...
void tTVPStringStream::write(tjs_char c) { stream->Write(&c, sizeof(c)); }
...
void tTVPStringStream::newline() { write(_newline); }

static void quoteString(const tjs_char *str, tTVPStringStream *writer) {
    if(str) {
...
public:
    DictMemberDispCaller(tTVPStringStream *writer) : writer(writer) {
        first = true;
    };
...
// Array クラスメンバ
static iTJSDispatch2 *ArrayCountProp = nullptr; // Array.count
...
static void getArrayString(iTJSDispatch2 *array, tTVPStringStream *writer) {
    ...
    tjs_int count = 0;
    {
        tTJSVariant result;
        if(TJS_SUCCEEDED(
               ArrayCountProp->PropGet(0, nullptr, nullptr, &result, array))) {
            count = result;
        }
    }
...
static void getVariantString(tTJSVariant &var, tTVPStringStream *writer) {
    switch(var.Type()) {
        ...
        case tvtObject: {
            ...
            if(obj == nullptr) {
                writer->write(TJS_W("nullptr"));
            } else if(obj->IsInstanceOf(TJS_IGNOREPROP, nullptr, nullptr,
                                        TJS_W("Array"), obj) == TJS_S_TRUE) {
...
public:
     ...
     * @return 実行結果
     */
    static tjs_error save2(tTJSVariant *result, tjs_int numparams,
     ...
     * @return 実行結果
     */
    static tjs_error saveStruct2(tTJSVariant *result, tjs_int numparams,
                                 tTJSVariant **param, iTJSDispatch2 *objthis) {
        if(numparams < 1)
            return TJS_E_BADPARAMCOUNT;
        tTJSBinaryStream *stream =
     ...
     * @return 実行結果
     */
    static tjs_error toStructString(tTJSVariant *result, tjs_int numparams,
                                    tTJSVariant **param,
...
static void PostRegistCallback() {
    // Array.count を取得
    {
        ...
        tTJSVariant val;
        if(TJS_FAILED(dispatch->PropGet(TJS_IGNOREPROP, TJS_W("count"), nullptr,
                                        &val, dispatch))) {
