    ...
    if(!_program) {
        ...
        if(!_backgroundListener) {
            ...
            cocos2d::Director::getInstance()
                ->getEventDispatcher()
                ->addEventListenerWithFixedPriority(_backgroundListener, -1);
        }
        _program = cocos2d::GLProgram::createWithByteArrays(
            cocos2d::ccPositionTextureColor_vert, _yuvRenderProgram_fsh);
    }
    return _program;
}
...
void TVPYUVSprite::setupVBOAndVAO() {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(_polyInfo.triangles.verts[0]) *
                     _polyInfo.triangles.vertCount,
                 _polyInfo.triangles.verts, GL_DYNAMIC_DRAW);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 sizeof(_polyInfo.triangles.indices[0]) *
                     _polyInfo.triangles.indexCount,
                 _polyInfo.triangles.indices, GL_DYNAMIC_DRAW);
...
void TVPYUVSprite::updateTextureDataInternal(
    cocos2d::Texture2D *pTex, const void *data, int width, int height,
...
void TVPYUVSprite::updateTextureData(const void *Y, int YW, int YH,
...
void TVPYUVSprite::onDraw() {
    ...
    //	glBufferData(GL_ARRAY_BUFFER,
    // sizeof(_polyInfo.triangles.verts[0]) *
    //_polyInfo.triangles.vertCount, nullptr, GL_DYNAMIC_DRAW);
    void *buf = glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
    memcpy(buf, _polyInfo.triangles.verts,
           sizeof(_polyInfo.triangles.verts[0]) * 4);
    ...
    cocos2d::Vec3 vertices[4] = { _polyInfo.triangles.verts[0].vertices,
    ...
    cocos2d::Color4B colors[4] = { _polyInfo.triangles.verts[0].colors,
    ...
    cocos2d::Tex2F texCoords[4] = { _polyInfo.triangles.verts[0].texCoords,
