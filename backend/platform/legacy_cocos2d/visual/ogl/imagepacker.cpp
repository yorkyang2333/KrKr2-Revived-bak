...
namespace ImagePacker {
    ...
    */
    ...
    struct node {
        ...
        node *insert(rect_xywhf &img) {
            ...
            switch(f) {
                ...
                case 3:
                    id = true; /*img.flipped = false;*/
                    return this;
                case 4:
                    id = true; /*img.flipped = true; */
                    return this;
            }

            int iw = img.w, ih = img.h;
            //		int iw = (img.flipped ? img.h : img.w), ih =
            //(img.flipped ? img.w : img.h);
    ...
    int rect_ltrb::w() const { return r - l; }

    int rect_ltrb::h() const { return b - t; }

    int rect_ltrb::area() const { return w() * h(); }

    int rect_ltrb::perimeter() const { return 2 * w() + 2 * h(); }

    void rect_ltrb::w(int ww) { r = l + ww; }

    void rect_ltrb::h(int hh) { b = t + hh; }
    ...
    int rect_xywh::r() const { return x + w; };

    int rect_xywh::b() const { return y + h; }

    void rect_xywh::r(int right) { w = right - x; }

    void rect_xywh::b(int bottom) { h = bottom - y; }

    int rect_wh::area() { return w * h; }
