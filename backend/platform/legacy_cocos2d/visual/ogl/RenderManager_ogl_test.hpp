...
#define TEST_SHADER(_method, ...)                                              \
    _TEST_SHADER(                                                              \
        #_method, this,                                                        \
        [&](std::vector<tRenderTexRectArray::Element> &src_tex,                \
            tTVPOGLRenderMethod *method) { __VA_ARGS__; },                     \
        false);
#define TEST_SHADER_IGNORE_ALPHA(_method, ...)                                 \
    _TEST_SHADER(                                                              \
        #_method, this,                                                        \
        [&](std::vector<tRenderTexRectArray::Element> &src_tex,                \
            tTVPOGLRenderMethod *method) { __VA_ARGS__; },                     \
...
static tTVPOGLTexture2D_mutatble *texdest = nullptr;
...
static void ResetTestData() {
    if(!texdest) {
        texdest = new tTVPOGLTexture2D_mutatble(nullptr, 0, 256, 256,
                                                TVPTextureFormat::RGBA, 1, 1);
        for(int x = 0; x < 256; ++x) {
            for(int y = 0; y < 256; ++y) {
                struct clr {
                    unsigned char a;
        ...
        texdata[2] = new tTVPOGLTexture2D_mutatble(
            testrule, 256, 256, 256, TVPTextureFormat::Gray, 1, 1);
        texdata[0] = new tTVPOGLTexture2D_mutatble(
            testdata1, 256 * 4, 256, 256, TVPTextureFormat::RGBA, 1, 1);
        texdata[1] = new tTVPOGLTexture2D_mutatble(
            testdata2, 256 * 4, 256, 256, TVPTextureFormat::RGBA, 1, 1);
        texred = new tTVPOGLTexture2D_mutatble(testred, 256 * 4, 256, 256,
                                               TVPTextureFormat::RGBA, 1, 1);
        texblue = new tTVPOGLTexture2D_mutatble(testblue, 256 * 4, 256, 256,
                                                TVPTextureFormat::RGBA, 1, 1);
    }
    memcpy(testdest, testdata2, 256 * 256 * 4);
    texdest->Update(testdata2, TVPTextureFormat::RGBA, 256 * 4,
                    tTVPRect(0, 0, 256, 256));
}

static void CheckTestData(const char *funcname) {
    const unsigned char *linetex =
        (const unsigned char *)texdest->GetScanLineForRead(0);
        ...
        if(std::abs(a1 - a2) > 3 ||
           (a1 > 3 &&
            (((std::abs(linetex[i * 4] - linecmp[i * 4]) * a1) >> 8) > 3 ||
             ((std::abs(linetex[i * 4 + 1] - linecmp[i * 4 + 1]) * a1) >> 8) >
                 3 ||
...
static void _TEST_SHADER(
    const char *_method, iTVPRenderManager *_this,
    const std::function<void(std::vector<tRenderTexRectArray::Element> &,
                             tTVPOGLRenderMethod *)> &fcall,
    bool ignoreAlpha) {
    tTVPOGLRenderMethod_Script *method =
        (tTVPOGLRenderMethod_Script *)_this->GetRenderMethod(_method);
    int id_opa = method->EnumParameterID("opacity"),
        id_clr = method->EnumParameterID("color");
    if(id_opa >= 0)
        method->SetParameterOpa(id_opa, TEST_SHADER_OPA);
    if(id_clr >= 0)
        method->SetParameterColor4B(id_clr, TEST_SHADER_COLOR);
    ResetTestData();
    ...
    _this->OperateRect(method, texdest, nullptr, rc,
                       src_tex.empty()
                           ? tRenderTexRectArray()
                           : tRenderTexRectArray(&src_tex[0], src_tex.size()));
    if(ignoreAlpha)
        _TEST_IGNORE_ALPHA();
