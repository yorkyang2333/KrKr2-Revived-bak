...
class HeapAllocAllocator : public iTVPMemoryAllocator
{
	...
	HANDLE HeapHandle;
public:
	HeapAllocAllocator() : HeapHandle(nullptr) {
		tTJSVariant val;
		tjs_uint64 size = 0;
		...
		if( HeapHandle == nullptr ) {
			...
			while( HeapHandle == nullptr && size > (1024*1024) ) {
	...
	void* allocate( size_t size ) {
		if( HeapHandle == nullptr ) return nullptr;
...
iTVPMemoryAllocator *tTVPBitmapBitsAlloc::Allocator = nullptr;
