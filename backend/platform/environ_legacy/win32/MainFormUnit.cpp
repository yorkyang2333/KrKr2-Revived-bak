...
#endif
#include "TickCount.h"
TTVPMainForm *TVPMainForm = nullptr;
...
// Global Definitions
//---------------------------------------------------------------------------
TTVPPadForm *TVPMainPadForm = nullptr;
TTVPConsoleForm *TVPMainConsoleForm = nullptr;
TTVPWatchForm *TVPMainWatchForm = nullptr;
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
#include "float.h"
__fastcall TTVPMainForm::TTVPMainForm(TComponent *Owner) : TForm(Owner) {
#ifndef __BORLANDC__
    init(this);
#endif
...
//---------------------------------------------------------------------------
void TTVPMainForm::NotifyConsoleHiding() { ShowConsoleButton->Down = false; }
//---------------------------------------------------------------------------
bool TTVPMainForm::GetScriptEditorVisible() {
    if(TVPMainPadForm)
        return TVPMainPadForm->Visible;
    return false;
}
//---------------------------------------------------------------------------
...
bool TTVPMainForm::GetWatchVisible() {
    if(TVPMainWatchForm)
        return TVPMainWatchForm->Visible;
    return false;
}
//---------------------------------------------------------------------------
...
void TTVPMainForm::NotifyEventDelivered() {
    ...
    LastCloseClickedTick = 0;
    if(TVPHaltWarnForm)
        delete TVPHaltWarnForm, TVPHaltWarnForm = nullptr;
}
//---------------------------------------------------------------------------
void TTVPMainForm::NotifyHaltWarnFormClosed() { LastCloseClickedTick = 0; }
//---------------------------------------------------------------------------
void TTVPMainForm::NotifySystemError() {
    if(AutoShowConsoleOnError)
...
//---------------------------------------------------------------------------
static tTVPProfileHolder *TVPEnvironProfile = nullptr;
...
static void TVPUninitEnvironProfile() {
    ...
    if(TVPEnvironProfile) {
        ...
        TVPEnvironProfile = nullptr;
    }
}
//---------------------------------------------------------------------------
void TVPEnvironProfileAddRef() { TVPEnvironProfileRefCount++; }
//---------------------------------------------------------------------------
void TVPEnvironProfileRelease() {
    TVPEnvironProfileRefCount--;
    if(TVPEnvironProfileRefCount == 0)
        TVPUninitEnvironProfile();
}
//---------------------------------------------------------------------------
...
void __fastcall tTVPProfileHolder::ReadStrings(const AnsiString &section,
    ...
    if(str != "")
        strings->Text = TVPUnescapeAnsiString(str);
}
//---------------------------------------------------------------------------
