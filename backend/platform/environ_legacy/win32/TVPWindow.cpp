...
// touch mouse message extraInfo (cf.
...
bool tTVPWindow::Initialize() { return true; }
...
int tTVPWindow::ShowModal() {
    ...
    if(Application->GetWindowCount() == 1) {
        // 1個しか
        // Windowがない時はModal化する意味がないのと、不具合の元なので例外発生
    ...
    try {
        ...
        try {
            Application->GetEnableWindowList(enableWindows, nullptr);
