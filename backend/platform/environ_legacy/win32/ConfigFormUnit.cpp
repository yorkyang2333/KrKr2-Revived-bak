...
private:
    ...
    void ClearOptionList() { ComboBox_ResetContent(OptionList); }
...
public:
    ConfigFormUnit(HWND hWnd) : WindowHandle(hWnd), CurrentItem(nullptr) {
    ...
    void SetCurrentItem(HTREEITEM hItem) {
        if(CurrentItem != nullptr && CurrentItem->ItemHandle == hItem)
            return;
...
static ConfigFormUnit *DialogForm = nullptr;
...
LRESULT WINAPI ConfigFormUnit::DlgProc(HWND hWnd, UINT msg, WPARAM wParam,
    ...
    if(msg == WM_INITDIALOG) {
        ...
        if(DialogForm)
            return DialogForm->Dispatch(hWnd, msg, wParam, lParam);
    }
    return FALSE;
}
//---------------------------------------------------------------------------
