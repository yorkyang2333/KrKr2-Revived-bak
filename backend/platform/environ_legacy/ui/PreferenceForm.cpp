...
tPreferenceItemCheckBox::tPreferenceItemCheckBox() : highlight(nullptr) {}
...
tPreferenceItemSelectList::tPreferenceItemSelectList() : highlight(nullptr) {}
...
void TVPCustomPreferenceForm::initFromInfo(
    ...
    _setter = setter;
    if(_title)
        LocaleConfigManager::GetInstance()->initText(_title, tid_title);
    if(!_listview)
        return;
