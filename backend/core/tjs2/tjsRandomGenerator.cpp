...
namespace TJS {
    //---------------------------------------------------------------------------
    void (*TJSGetRandomBits128)(void *dest) = nullptr;
    ...
    iTJSDispatch2 *tTJSNI_RandomGenerator::Serialize() {
        ...
        try {
            ...
            val = state;
            dic->PropSet(TJS_MEMBERENSURE, TJS_W("state"), nullptr, &val, dic);

            val = (tjs_int)data.left;
            dic->PropSet(TJS_MEMBERENSURE, TJS_W("left"), nullptr, &val, dic);

            val = (tjs_int)(data.next - data.state);
            dic->PropSet(TJS_MEMBERENSURE, TJS_W("next"), nullptr, &val, dic);
        } catch(...) {
            if(dic)
                dic->Release();
            throw;
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ random63) {
    ...
    // returns 63-bit precision integer value x, x is in 0 <= x <=
    // 9223372036854775807
