...
//---------------------------------------------------------------------------
static HMIDIOUT TVPMIDIOutHandle = nullptr;
...
public:
    tTVPMIDIThread() : tTVPThread(true) { SetPriority(ttpTimeCritical); };
...
//---------------------------------------------------------------------------
tTVPSMFTrack::~tTVPSMFTrack() { delete[] Data; }
//---------------------------------------------------------------------------
void tTVPSMFTrack::Rewind() {
    Position = Data;
    Ended = false;
    PrevEvent = 0;
...
//---------------------------------------------------------------------------
tjs_error tTJSNI_MIDISoundBuffer::Construct(tjs_int numparams,
                                            tTJSVariant **param,
...
void tTJSNI_MIDISoundBuffer::WndProc(Messages::TMessage &Msg) {
    // called by OS, caused by posting of status message from playing
    // thread.
...
void tTJSNI_MIDISoundBuffer::AllNoteOff() {
    ...
    for(i = 0; i < 16; i++) {
        if(UsingChannel[i]) {
            TVPMIDIOut((tjs_uint8)(0xb0 + i), 0x7B,
                       0x00); // All Note Off
