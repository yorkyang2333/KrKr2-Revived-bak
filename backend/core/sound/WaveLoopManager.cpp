...
bool tTVPWaveLoopManager::GetString(char *s, tTVPLabelStringType &v) {
    ...
    tjs_int size = TVPUtf8ToWideCharString(s, nullptr);
    if(size == -1)
        return false; // not able to convert the string
...
void tTVPWaveLoopManager::PutString(AnsiString &s, tTVPLabelStringType v) {
    ...
    // count output bytes
    int size = TVPWideCharToUtf8String(pi, nullptr);
