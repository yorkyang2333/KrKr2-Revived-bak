...
void tTVPWaveSegmentQueue::Dequeue(tTVPWaveSegmentQueue &dest,
    ...
    while(Segments.size() > 0 && remain > 0) {
        if(Segments.front().FilteredLength <= remain) {
            ...
            if(newlength > 0)
                dest.Enqueue(
                    tTVPWaveSegment(Segments.front().Start, newlength, remain));

            // Segments.front() の Start, Length と FilteredLength
            // を修正
    ...
    while(Labels_to_dequeue--)
        Labels.pop_front(); // コピーしたLabels を削除
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
tjs_int64
tTVPWaveSegmentQueue::FilteredPositionToDecodePosition(tjs_int64 pos) const {
    // Segments の修正
    ...
    return Segments[Segments.size() - 1].Start +
        Segments[Segments.size() - 1].Length;
}
//---------------------------------------------------------------------------
