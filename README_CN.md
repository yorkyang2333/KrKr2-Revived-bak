...
- **渲染/输入**：实现全新 `IWindow` 和 `IRenderer` 抽象接口层，配套 SDL3 平台实现（`SDLWindow`, `SDLRenderer`, `SDLInput`）。`WindowImpl.cpp` 全量迁移到新接口。
- **原生游戏免改容错**：可直接运行原版被打包的 `.xp3` 游戏本身，并提供运行时自动化解密匹配推断及 Windows 缺省平台 API 容错。
...
根目录 `.gitignore` 已经忽略了这些生成内容，正常情况下不会污染 `git status`。
...
├── frontend/                  ← Flutter 启动器 UI 与 FFI 桥接层
├── doc/                       ← 文档
├── scripts/                   ← 构建脚本
├── tests/                     ← 测试
└── CMakeLists.txt
...
- [x] 定义 `IWindow` 抽象接口（`backend/core/visual/IWindow.h`）——包含几何/全屏/光标/IME/触摸/缩放等全量方法
- [x] 定义 `IRenderer` 抽象接口（`backend/core/visual/IRenderer.h`）
- [x] `SDLWindow` SDL3 实现——位置/最小最大尺寸/全屏/光标/置顶显示/关闭
- [x] `SDLRenderer` SDL3 实现
- [x] `SDLInput` SDL3 事件轮询 → 引擎路由
- [x] `TVPWindow_SDL.cpp` —— SDL 平台工厂（`TVPCreateAndAddWindow`）
